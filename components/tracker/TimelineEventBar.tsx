...
export function TimelineEventBar({
  ...
  const width = Math.max(dayWidth - 18, getEventDuration(startDate, endDate) * dayWidth - 24);
  const statusClass =
    event.status === "earned"
  ...
  return (
    <button
      ...
      className={`absolute flex h-11 items-center justify-between gap-2 rounded border px-3 text-left text-xs shadow-lg transition hover:-translate-y-0.5 hover:border-yellow-300 ${statusClass}`}
      style={{ left, top, width }}
    >
      <span className="min-w-0">
        <span className="block truncate font-black">{event.name}</span>
        <span className="block truncate text-[11px] opacity-80">{event.type} · {event.status}</span>
      </span>
